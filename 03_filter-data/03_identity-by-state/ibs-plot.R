...
### read results
ibs <- readr::read_table("03_filter-data/03_identity-by-state/merged.mibs",
                         col_names = F) %>%
  as.data.frame()

### read ids
ids <- readr::read_table("03_filter-data/03_identity-by-state/merged.mibs.id",
                         col_names = F) %>%
...
ibs_tri_mean <- ibs_tri %>%
  dplyr::mutate(population1 = factor(population1,
                ...
                group1 = dplyr::case_when(population1 %in% c("HOLS", "ANGU", "JERS") ~ "European Bos taurus",
                                          ...
                                          population1 %in% c("MUTU", "LAGU", "NDAG", "BAOU") ~ "African Bos taurus",
                                          population1 %in% c("NDAM", "BORG", "SOMB", "KETE", "SHEK") ~ "Trypanotolerant African hybrid",
                                          population1 %in% c("FULA", "ANKO", "NGAN", "EASZ", "KARA", "BORA") ~ "Trypanosusceptible African hybrid",
                ...
                group2 = dplyr::case_when(population2 %in% c("HOLS", "ANGU", "JERS") ~ "European Bos taurus",
                                          ...
                                          population2 %in% c("MUTU", "LAGU", "NDAG", "BAOU") ~ "African Bos taurus",
                                          population2 %in% c("NDAM", "BORG", "SOMB", "KETE", "SHEK") ~ "Trypanotolerant African hybrid",
                                          population2 %in% c("FULA", "ANKO", "NGAN", "EASZ", "KARA", "BORA") ~ "Trypanosusceptible African hybrid",
                ...
                group2 = factor(group2,
                                labels = group_labels,
                                levels = group_levels)) %>%
  dplyr::group_by(population1, population2) %>%
