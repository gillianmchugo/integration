...
ticks <- dplyr::tibble(FID = lims,
  ...
  dplyr::mutate(group = dplyr::case_when(FID %in% c("HOLS", "ANGU", "JERS") ~ "European Bos taurus",
                                         ...
                                         FID %in% c("MUTU", "LAGU", "NDAG", "BAOU") ~ "African Bos taurus",
                                         FID %in% c("NDAM", "BORG", "SOMB", "KETE", "SHEK") ~ "Trypanotolerant African hybrid",
                                         FID %in% c("FULA", "ANKO", "NGAN", "EASZ", "KARA", "BORA") ~ "Trypanosusceptible African hybrid",
                                         FID %in% c("THAR", "GIR", "NELO") ~ "Bos indicus"))

### read and plot inbreeding data
plot <- readr::read_table("03_filter-data/04_inbreeding/merged-ibs.het") %>%
  dplyr::group_by(FID) %>%
  dplyr::mutate(group = dplyr::case_when(FID %in% c("HOLS", "ANGU", "JERS") ~ "European Bos taurus",
                                         FID %in% c("ALEN", "CHIA", "MARC", "MARE", "ROMA") ~ "European hybrid",
                                         FID %in% c("MUTU", "LAGU", "NDAG", "BAOU") ~ "African Bos taurus",
                                         FID %in% c("BORG", "KETE", "NDAM", "SHEK", "SOMB") ~ "Trypanotolerant African hybrid",
                                         FID %in% c("FULA", "ANKO", "BORA", "EASZ", "KARA", "NGAN") ~ "Trypanosusceptible African hybrid",
  ...
  ggplot2::geom_boxplot(fill = NA,
                        outlier.size = 2.5) +
  ggplot2::geom_dotplot(ggplot2::aes(colour = FID, fill = FID),
                        binaxis = "y",
                        binwidth = 0.003,
                        dotsize = 5,
                        stackdir = "center",
...
### save plot
png("03_filter-data/04_inbreeding/inbreeding.png",
    height = 6,
    res = 300,
    units = "in",
    width = 9)
print(plot)
dev.off()
pdf("03_filter-data/04_inbreeding/inbreeding.pdf",
    height = 6,
    width = 9)
print(plot)
dev.off()